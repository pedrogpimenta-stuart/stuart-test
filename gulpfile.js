...
gulp.task('html', function() {
  ...
  .source('src/content')
  .destination('../pedrogpimenta-stuart.github.io/')
...
gulp.task('serve', ['renameNormalize', 'sass', 'html'], function() {
  if (devBuild) {
    browserSync.init({ // Start BrowserSync server
      server: {
        baseDir: '../pedrogpimenta-stuart.github.io/'
      }
    });
    gulp.watch('src/sass/**/*.s*ss', ['sass']); // Run sass task
    gulp.watch('src/**/*.+(html|md|js)', ['html']); // Run Metalsmith
    gulp.watch('../pedrogpimenta-stuart.github.io//**/*.html').on('change', browserSync.reload); // Reload BrowserSync if HTMLs have changed
  }
});
